...
public class Browser {
    ...
    public Browser(WebDriver providedDriver) {
        ...
        webDriverWait.set(new WebDriverWait(driver.get(), Duration.ofSeconds(MainConfig.getLongProperty(MainConfigProperty.webDriverExplicitWaitInSeconds))));
    ...
    public void setImplicitWaitTimeout(long timeoutInSeconds) {
        debug(String.format("Setting driver implicit wait time to %s seconds", timeoutInSeconds));
        driver.get().manage().timeouts().implicitlyWait(Duration.ofSeconds(timeoutInSeconds));
    }

    public void resetImplicitWaitTimeout() {
        long value = MainConfig.getLongProperty(MainConfigProperty.webDriverImplicitWaitInSeconds);
        debug(String.format("Resetting driver implicit wait time to %s seconds", value));
    ...
    public void waitForElementToVanish(WebElement element, String description) {
        try {
            info("waiting for element to vanish: " + description);
            webDriverWait.get().until(ExpectedConditions.invisibilityOf(element));
            info("Element vanished! Moving on...");
        } catch (StaleElementReferenceException e) {
            info("Element vanished!");
        }
    }
    ...
    public void waitForPageStable(Duration duration) {
        debug("Waiting for page to become stable - by no body changes between several seconds");
        ...
        if (duration == null) {
            ...
            waiter = new WebDriverWait(driver.get(), Duration.ofSeconds(MainConfig.getLongProperty(MainConfigProperty.webDriverExplicitWaitInSeconds)))
        ...
        waiter.until(webDriver -> {
            try {
                ...
                debug("Error occur trying to wait for page stable: " + ExceptionUtils.getStackTrace(e));
                return false;
            }
        });
        debug("Page now seems stable!");
    }
    ...
    public WebElement click(ExtendedBy by, ExecuteBy executeBy, boolean forceElementOnTop) {
        ...
        try {
            ...
            debug("A StaleElementReferenceException occur trying to click element: " + by.toString() + ". Trying again. Details: " + e);
            webElement = webDriverWait.get().until(ExpectedConditions.elementToBeClickable(by.getBy()));
            click(webElement, by.getDescription(), executeBy, forceElementOnTop);
    ...
    public void typeTextElement(ExtendedBy extendedBy, String text) {
        ...
        for (int i = 1; i <= MAX_STALE_ELEMENT_RETRIES; i++) {
            try {
                ...
                debug("A StaleElementReferenceException occur trying to send text to element: " + extendedBy.getDescription() + " trying again... (" + i + "/" + MAX_STALE_ELEMENT_RETRIES + ")");
    ...
    public void sendKeysElement(ExtendedBy extendedBy, String keysDescription, CharSequence... keys) {
        info("Sending keys: " + keysDescription + " to " + extendedBy.getDescription());
        WebElement element = driver.get().findElement(extendedBy.getBy());
    ...
    private void typeTextElement(WebElement element, String description, String text, boolean isSecureField) {
        ...
        for (int i = 1; i <= MAX_ACTION_ATTEMPTS; i++) {
            try {
                ...
                info("About to send text '" + textToShow + "' on element: " + description);

                scrollToElement(element, false);

                info("Clearing text in element " + description);
                element.clear();

                element.sendKeys(text);
                info("Finished sending text '" + textToShow + "' to element: " + description);
                break;
            } catch (Exception e) {
                if (i < MAX_ACTION_ATTEMPTS - 1) {
                    info("[WARNING]: Send text to element " + description + " failed (" + e + ") . Trying again in " + SLEEP_IN_MILLIS_AFTER_ACTION_FAIL + " " + MILLISECONDS_LITERAL);
                    sleep(SLEEP_IN_MILLIS_AFTER_ACTION_FAIL);
                } else {
                    throw e;
                }
    ...
    public void sendTextElementIncTrimDecimals(ExtendedBy extendedBy, String text) {
        WebElement element = driver.get().findElement(extendedBy.getBy());
        info("About to send text '" + text + "' on element: " + extendedBy.getDescription());
        element.sendKeys(text);
        //delete 3 chars: ".00"
        ...
        info("Finished sending text '" + text + "' to element: " + extendedBy.getDescription());
    }
    ...
    public String getElementText(ExtendedBy extendedBy) {
        ...
        try {
            ...
            debug("Stale element issue occur reading text of element " + extendedBy.getDescription() + ". Trying again...");
            WebElement webElement = driver.get().findElement(extendedBy.getBy());
            return getElementText(webElement, extendedBy.getDescription());
        }
    }
    ...
    public String getElementText(WebElement webElement, String elementDescription) {
        ...
        info(elementDescription + " text: '" + text + "'");
        return text;
    }
    ...
    public void selectElementByVisibleText(ExtendedBy extendedBy, String value) {
        ...
        info("selected value " + value + " in element: " + extendedBy.getDescription());
    }
    ...
    public String selectRandomValueInElement(ExtendedBy extendedBy, boolean allowFirstValue) {
        for (int i = 0; i < 5; i++) {
            try {
                ...
                info("selected random value of " + selectedText + " in element: " + extendedBy.getDescription());
                return selectedText;
            } catch (StaleElementReferenceException e) {
                debug("A StaleElementReferenceException occur trying to select random value in element " + extendedBy.toString() + ". Trying again. Details: " + e);
            }
        }
    ...
    public void waitForElementVisibility(ExtendedBy extendedBy, int timeoutSeconds) {
        info("Waiting for '" + extendedBy.getDescription() + "' to be visible by: " + extendedBy.getBy() + " waiting up to " + timeoutSeconds + " seconds");
    ...
    public void waitForElementClickable(ExtendedBy extendedBy, int timeoutSeconds) {
        info("Waiting for '" + extendedBy.getDescription() + "' to be clickable by: " + extendedBy.getBy() + " waiting up to " + timeoutSeconds + " seconds");
        var wait = new WebDriverWait(driver.get(), Duration.ofSeconds(timeoutSeconds));
    ...
    public WebElement waitForElementVisibility(ExtendedBy extendedBy) {
        info("Waiting for '" + extendedBy.getDescription() + "' to be visible by: " + extendedBy.getBy());
    ...
    public void waitForElementVisibleCount(ExtendedBy extendedBy, int expectedCount) {
        info("Waiting for '" + extendedBy.getDescription() + "' visible instances to be: " + expectedCount + " finding them by: " + extendedBy.getBy());
        webDriverWait.get().until(CustomConditions.numberOfVisibleElementsToBe(extendedBy.getBy(), expectedCount));
    }


    public void waitForElementTextNotEmpty(ExtendedBy extendedBy) {
        info("Waiting for '" + extendedBy.getDescription() + "' text to not be null. Finding it by: " + extendedBy.getBy());
    ...
    public WebElement waitForElementClickable(ExtendedBy extendedBy) {
        info("Waiting for '" + extendedBy.getDescription() + "' not to be clickable by: " + extendedBy.getBy());
    ...
    public void waitForElementTextValueNotZero(ExtendedBy extendedBy) {
        info("Waiting for '" + extendedBy.getDescription() + "' text value to be not zero. Finding it by: " + extendedBy.getBy());
        webDriverWait.get().until((ExpectedCondition<Boolean>) d -> {
            try {
                String currentValueAsString = getElementText(extendedBy);
                info("Current value is: " + currentValueAsString);
    ...
    public void waitForElementToHaveAttribute(WebElement element, String description, String attributeName) {
        info("Waiting for element '" + description + "' to have an attribute named: " + attributeName);
        webDriverWait.get().until((ExpectedCondition<Boolean>) d -> {
            try {
    ...
    public void scrollToElement(WebElement element, boolean forceElementOnTop) {
        ...
        try {
            if (forceElementOnTop) {
                ...
                var wait = new WebDriverWait(driver.get(), Duration.ofSeconds(MainConfig.getLongProperty(MainConfigProperty.webDriverExplicitWaitInSeconds)));
            ...
            debug("An error occur trying to scroll to element " + element + ". Details: " + e);
        }
    }
    ...
    public void sleep(long milliseconds) {
        info("Sleeping for " + milliseconds + " " + MILLISECONDS_LITERAL);
        try {
    ...
    public String getElementAttribute(ExtendedBy locator, String attributeName) {
        ...
        info("For element " + locator.getDescription() + " attribute " + attributeName + "=" + result);
        return result;
    }
    ...
    public static String getScreenshotAsBase64() {
        if (driver.get() != null) {
            ...
            warning("Can't take screenshot when driver is not initialized", false);
            return null;
        }
    }
    ...
    public static String getBrowserConsoleErrors() {
        if (driver.get() == null) {
            warning("Can't get browser console errors when driver is not initialized", false);
            return null;
        }
    ...
    public void refreshPage(boolean validateUrlDidNotChange, boolean waitForPageStable) {
        ...
        debug("Adding element in page so we know refresh is done (once element no longer exists)");
        ...
        info("Refreshing current page... (" + originalUrl + ")");
    ...
    public int getNumberOfOpenWindows() {
        ...
        info("There are " + result + " open windows/tabs");
        return result;
    }
    ...
    public void waitForElementCssAttributeToEqual(WebElement element, String cssAttributeName, String expected, int timeoutSeconds) {
        info("Waiting for css attribute of " + cssAttributeName + " to become: " + expected);
        Function<String, Boolean> internalWaitForElementAttributeToEqual = t -> {
            ...
            debug("css attribute " + cssAttributeName + " expected value: " + expected + " actual value: " + attributeValue);
    ...
    public WebElement waitForElementToBeClickable(ExtendedBy extendedBy, int timeoutSeconds) {
        info("Waiting for '" + extendedBy.getDescription() + "' to be clickable by: " + extendedBy.getBy());
        return webDriverWait.get().until(ExpectedConditions.elementToBeClickable(extendedBy.getBy()));
    }

    public void switchToFirstWindow() {
        info("Switching back to the original window/tab");
    ...
    public void switchToLatestWindow() {
        ...
        for (int i = 1; i <= maxAttempts; i++) {
            try {
                info("Switching to last (new) tab - attempt #" + i);
                ArrayList<String> tabs = new ArrayList<>(driver.get().getWindowHandles());
                driver.get().switchTo().window(tabs.get(tabs.size() - 1));
                return;
            } catch (NoSuchWindowException e) {
                info("Error occur trying to switch to latest window.. Will try again in 2 seconds. Details: " + e);
                sleep(2000);
            }
        }
    ...
    public void openNewTabAndSwitchToIt(String url) {
        ...
        Failsafe.with(retryPolicy).run(() -> {
            ...
            info("Opening new tab in url: " + finalUrl);
    ...
    private String executeScriptInternal(String script, boolean isLogResponse, Object... objects) {
        if (isLogResponse) {
            info("Executing js script: " + script);
        }
        Object response = javascriptExecutor.get().executeScript(script, objects);
        if (response != null) {
            String responseAsString = response.toString();
            if (isLogResponse) {
                info("Finished executing js script with result: " + responseAsString);
            } else {
                debug("Finished executing js script");
            }
    ...
    public void closeLastTabAndSwitchToPreviousOne() {
        info("Closing current tab opened in: " + driver.get().getCurrentUrl());
        ArrayList<String> tabs = new ArrayList<>(driver.get().getWindowHandles());
        driver.get().switchTo().window(tabs.get(tabs.size() - 1));
        driver.get().close();
    ...
    public static <T, R> void waitForExpressionToEqual(Function<T, R> func, T param, R expected, String description, int timeoutSeconds, int checkIntervalMilliseconds, RuntimeException exceptionToThrowOnTimeout) {
        ...
        do {
            ...
            if (result.equals(expected)) {
                info("waitForExpressionToEqual (" + description + "): Finished! Condition met in " + (System.currentTimeMillis() - startTime) / 1000 + " seconds");
                return;
            }

            long passedTime = System.currentTimeMillis() - startTime;
            waitTimePercentage = (passedTime * 100.0f) / (timeoutSeconds * 1000);
            debug("waitForExpressionToEqual (" + description + "): Condition not met yet.. Trying again in " + checkIntervalMilliseconds + " milliseconds. (Max wait time passed: " + waitTimePercentage + "% out of " + timeoutSeconds + " allowed seconds)");

            try {
                Thread.sleep(checkIntervalMilliseconds);
            } catch (InterruptedException e) {
                warning("waitForExpressionToEqual (" + description + "): Interrupted!");
                Thread.currentThread().interrupt();
                return;
            }
    ...
    public static <T, R> R waitForExpressionNotNull(Function<T, R> func, T param, String description, int timeoutSeconds, int checkIntervalMilliseconds) {
        ...
        do {
            R result = func.apply(param);
            if (result != null) {
                info("waitForExpressionNotNull (" + description + "): Finished! Condition met in " + (System.currentTimeMillis() - startTime) / 1000 + " seconds");
                return result;
            }

            long passedTime = System.currentTimeMillis() - startTime;
            waitTimePercentage = (passedTime * 100.0f) / (timeoutSeconds * 1000);
            info("waitForExpressionNotNull (" + description + "): Condition not met yet.. Trying again in " + checkIntervalMilliseconds + " milliseconds. (Max wait time passed: " + waitTimePercentage + "% out of " + timeoutSeconds + " allowed seconds)");

            try {
                Thread.sleep(checkIntervalMilliseconds);
            } catch (InterruptedException e) {
                warning("waitForExpressionNotNull (" + description + "): Interrupted!");
                Thread.currentThread().interrupt();
                return null;
            }
    ...
    public void printPageSource() {
        ...
        info("Current page HTML: " + content);
        ...
        for (WebElement currentFrame : frames) {
            ...
            info("Current frame HTML: " + content);
            driver.get().switchTo().defaultContent();
        }
    }
    ...
    public WebElement setFocusElement(ExtendedBy by) {
        info("Setting element " + by.getDescription() + " as selected element (focus)");
        waitForElementVisibility(by);
        WebElement element = findElement(by);
    ...
    public boolean isElementEnabled(ExtendedBy extendedBy) {
        info("Checking if element " + extendedBy.getDescription() + " is enabled");
        WebElement element;
        for (int i = 1; i <= MAX_STALE_ELEMENT_RETRIES; i++) {
            try {
                ...
                debug("A StaleElementReferenceException occur trying to check if element is enabled: " + extendedBy.getDescription() + " trying again... (" + i + "/" + MAX_STALE_ELEMENT_RETRIES + ")");
                sleep(SLEEP_IN_MILLIS_AFTER_ACTION_FAIL);
            }
        }
        ...
        Function<String, String> waitForTextBeEqual = t -> {
            try {
                ...
                if (!actual[0].equals(expected)) {
                    info(elementDescription+" is not as "+expected+" expected. It is "+ actual[0]);
                    refreshPage();
                }
            } catch (Exception e) {
    ...
    public boolean verifyAlertMessage(ExtendedBy extendedBy, String message, boolean isMessageShouldShow) {
        ...
        if (isMessageShouldShow) {
            ...
            if (isElementVisible(extendedBy)) {
                ...
                if (actualMessage.contains(message)) {
                    ...
                    info("Message shown and it is as expected" +
                            "The message is : " + actualMessage);
                } else {
                    info("Message shown but it is not as expected" +
                            "Expected: " + message + " " +
                            "Actual: " + actualMessage);
                }
            } else {
                info("Message is not shown and it should");
            }
        }
        // Bulk of condition if message should NOT show
            ...
            if (isElementVisible(extendedBy)) {
                isMessageShow = true;
                actualMessage = getElementText(extendedBy);
                info("Message is shown and it should NOT");
                info("The message is : " + actualMessage);
            } else {
                info("Procedure succeed, there is no error message");
            }
        }
    ...
    public boolean isMobileDriver() {
        return WebDriverFactory.getOriginalDriver() instanceof IOSDriver;
    }

    private IOSDriver getIosOriginalDriver() {
        WebDriver originalDriver = WebDriverFactory.getOriginalDriver();
        if (originalDriver instanceof IOSDriver) {
            return ((IOSDriver) originalDriver);
        }
        throw new ClassCastException("There is no original IOS driver");
    }

    public Set<String> getContexts() {
        return getIosOriginalDriver().getContextHandles();
    }

    private void switchToContext(String name) {
        Set<String> contexts = getContexts();
        Optional<String> context = contexts.stream().filter(x -> x.toUpperCase().contains(name)).findFirst();
        if (context.isPresent()) {
            getIosOriginalDriver().context(context.get());
        } else {
            errorAndStop("No " + name + " context detected", true);
        }
    }
    ...
    public void switchToWebViewContext() {
        // wait for the webView context to exist
        waitForExpressionToEqual(q -> getContexts().stream().anyMatch(x -> x.toUpperCase().contains("WEBVIEW")),
                null,
                true,
                "wait for the WEBVIEW context to exist",
                30,
