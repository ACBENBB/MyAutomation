...
public class AUT484_ATS_Match_Order_Sell_Side {

    @Test(description = "Creates a 'Sell' order and match it with a different user.", groups = {"apiSanityATS"})
    @AtsMarketDependent(expectedState = AtsMarketState.UP)
    public void AUT484_ATS_Match_Order_Sell_Side() throws InterruptedException {
        ...
        String price = "1";
        String quantity = "1";
        HashMap<String, String> buyOrder = user1.createOrder(tokenName, Orders.SIDE.S.name(), quantity, price);
        ...
        HashMap<String, String> sellOrder = user2.createOrder(tokenName, Orders.SIDE.B.name(), quantity, price);
        ...
        String afterMatchSellStatus = ATSOrdersQuery.findOrderStatusByOrderIdAndReferenceNumber(
                sellOrder.get(Orders.PROPERTIES.orderId.name()),
                sellOrder.get(Orders.PROPERTIES.referenceNumber.name()));

        Assert.assertEquals(
                afterMatchSellStatus,
                Orders.STATUS.Executed.name(),
